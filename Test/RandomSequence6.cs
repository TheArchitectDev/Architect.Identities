...
namespace Test
{
	/// <summary>
	/// <para>
	/// A sequence of 6 bytes containing 45 bits of pseudorandom data, optionally increased to up to 48 bits by repeatedly adding another 40 random bits.
	/// This type is castable to a ulong with the high 16-19 bits set to zero.
	/// </para>
	/// <para>
	/// A newly created instance will have 45 bits of random data.
	/// By calling <see cref="Add40RandomBits(RandomSequence6)"/> [up to <see cref="MaxSubsequentAddOperations"/> times], 40 bits are added repeatedly.
	/// </para>
	/// <para>
	...
	{
		private const ulong MaxValue = UInt64.MaxValue >> (64 - InitialBitCount);
		/// <summary>
		/// The number of bits added by the <see cref="AddRandomBits(RandomSequence6)"/> operation.
		/// </summary>
		private const int AdditionalBitCount = 41;
		private const int InitialBitCount = 48;
		/// <summary>
		...
		private RandomSequence6(byte _)
		{
			...
			this._value &= ~0UL >> (64 - InitialBitCount);

			if (this._value == 0UL) this._value = 1UL; // Avoid 0, which would result in exeptions
